...
public class NoteService {
    ...
    public ResponseEntity<ResponseMessage> shareNote(final ShareForm form, Long parentUserId) throws RepositoryException, ServiceException {
        ...
        try {
            if (userRepository.isEmailExists(userDetails)) {
                ...
                if (repository.isNoteBelongToUser(parentNoteId, whoShare.getUser_id())) {
                    ...
                    if (repository.isNoteAlreadyShared(curNoteIdNextUser)) {
                        ...
                        return new ResponseEntity<>(new ResponseMessage(false, "Note is already shared to user."), HttpStatus.NOT_ACCEPTABLE);
                    }
                    ...
                    return new ResponseEntity<>(new ResponseMessage(false, "Can't share not own note."), HttpStatus.NOT_ACCEPTABLE);
                }

            } else {
                LOG.warn(String.format("Email(%s) is not found.", userDetails.getUsername()));
                return new ResponseEntity<>(new ResponseMessage(false, "Email is not found!"), HttpStatus.NOT_FOUND);
            }
        } catch (Exception e) {
            ...
            txManager.rollback(status);
            return new ResponseEntity<>(new ResponseMessage(false, "An error occurred while sharing note: " + e.getMessage()), HttpStatus.NOT_FOUND);
        }
    }
    ...
    public void updateOrder(final OrderData orderData) throws ServiceException {
        ...
        try {
            ...
            txManager.rollback(status);
            throw new ServiceException("An error occurred while saving note order.к" + e.getMessage());
        }
    }
